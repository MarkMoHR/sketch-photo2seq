...
def to_normal_strokes_4to3(stroke4s):
    """Convert from stroke-4 format back to stroke-3."""
    assert type(stroke4s) is list

    stroke3s = []
    for stroke4 in stroke4s:
        stroke3 = np.zeros((stroke4.shape[0], 3), dtype=np.float32)
        stroke3[:, 0:3] = stroke4[:, 0:3]
        stroke3s.append(stroke3)
...
class DataLoader(object):
    ...
    def __init__(self,
        ...
        # sorted by size)
        self.preprocess(strokes, img_paths)
        self.img_h = img_h
        self.img_w = img_w
    ...
    def preprocess(self, strokes, img_paths):
        ...
        for i in range(len(seq_len)):
            ...
            self.img_paths.append(raw_data_paths[idx[i]])
    ...
    def _get_batch_from_indices(self, indices):
        ...
        for idx in range(len(indices)):
            ...
            img_paths.append(self.img_paths[i])
        seq_len = np.array(seq_len, dtype=int)

        # We return three things: stroke-3 format, stroke-5 format, list of seq_len.
        return x_batch, self.pad_batch(x_batch), seq_len, self.load_images(img_paths), img_paths
    ...
    def load_images(self, image_paths):
        ...
        img_batch = np.zeros(shape=[self.batch_size, self.img_h, self.img_w, 3], dtype=np.float32)

        for img_idx in range(len(image_paths)):
            image_path = image_paths[img_idx]
            image = Image.open(image_path).convert('RGB')
            if image.height != self.img_h or image.width != self.img_w:
